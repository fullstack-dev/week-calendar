...
export class HomePage {
  ...
  getDaysOfMonth() {
    ...
    for(var i = prevNumOfDays-(firstDayThisMonth-1); i <= prevNumOfDays; i++) {
      ...
      if(this.date.getMonth() == 0) {
        month = this.monthNames[11];
        year = this.date.getFullYear() - 1;
      }
      this.daysTotal.push({day: i, month: month, year: year})
    }
    ...
    for (var i = 0; i < thisNumOfDays; i++) {
      ...
      let year = this.date.getFullYear();
      this.daysTotal.push({day: i+1, month: month, year: year});
    }
  ...
  selectDate(day) {
    ...
    if(this.oneSelectedDate == day.day && this.oneSelectedMonth == day.month && this.oneSelectedYear == day.year) {
      this.oneSelectedDate = '';
      this.oneSelectedMonth = '';
      this.oneSelectedYear = '';
    } else {
      this.oneSelectedDate = day.day;
      this.oneSelectedYear = day.year;
